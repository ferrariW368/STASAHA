  ...
  return (
    <main className="mx-auto max-w-lg px-4 py-6">
      ...
      <ol className="mb-4 flex flex-col gap-2">
        {ranked.map((u, i) => (
          <li key={u.username} className="flex items-center justify-between rounded-xl bg-neutral-900 p-3 shadow-sm">
            <div>
            ...
            <div className={`font-bold ${u.score.net > 0 ? 'text-green-400' : u.score.net < 0 ? 'text-red-400' : 'text-neutral-500'}`}>
              {u.score.net > 0 ? '+' : ''}{u.score.net} puan
            </div>
          </li>
        ))}
        {ranked.length === 0 && (
          <li className="rounded-xl bg-neutral-900 p-4 text-center text-sm text-neutral-600 shadow-sm">Henüz kullanıcı yok.</li>
        )}
      </ol>

      <div className="rounded-xl bg-neutral-900 p-4 text-xs text-neutral-500 shadow-sm">
        <p className="mb-1 font-semibold text-neutral-300">Puan nasıl hesaplanır?</p>
        <p>
