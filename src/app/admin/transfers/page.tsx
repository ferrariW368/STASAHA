      ...
      <div className="flex flex-col gap-3">
        {news.map((n) => (
          <div key={n.id} className="rounded border p-3 text-sm">
            ...
            {n.note && <p className="mb-2 text-xs text-gray-500">{n.note}</p>}
            <form
              action={async (formData) => {
                'use server';
                const stage = formData.get('stage') as string;
            ...
            >
              ...
              <button className="rounded bg-gray-700 px-3 py-1 text-xs font-semibold text-white">Güncelle</button>
            </form>
          </div>
        ))}
        {news.length === 0 && <p className="text-sm text-gray-400">Henüz transfer haberi yok.</p>}
      </div>
    </div>
  );
}
