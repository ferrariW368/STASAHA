...
- All text inputs/selects/textareas: `rounded-lg border border-line bg-pitch-night px-3 py-2 text-sm text-text-primary`. **Exception:** compact in-card rows (e.g. the per-team player-add row, per-transfer update row) may use a denser `px-2 py-1` on inputs and `px-3 py-1` on buttons — this compact variant is intentional wherever a form is nested one level inside an already-bordered card, not a deviation to fix.
